...
def show_settings():
    st.markdown(
        "<h1 style='text-align: center;'>⚙️ Settings</h1>", unsafe_allow_html=True
    )
    ...
    connection_string = st.text_input(
        "Enter your connection string", value=connection_string, type="password"
    )

    container_names = (
        fetch_container_names(connection_string) if connection_string else []
    )
    if container_names:
        container_name = st.selectbox(
            ...
            container_names,
            index=(
                container_names.index(container_name)
